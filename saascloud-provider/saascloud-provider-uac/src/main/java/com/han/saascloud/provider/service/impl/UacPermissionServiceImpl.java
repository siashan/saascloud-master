...
import com.han.saascloud.provider.security.SecurityUtils;
import com.han.saascloud.provider.service.UacPermissionService;
...
public class UacPermissionServiceImpl implements UacPermissionService {
	...
	public boolean hasPermission(Authentication authentication, HttpServletRequest request) {
		...
		log.info("验证权限loginName={}, requestURI={}, hasAuthorityUrl={}", currentLoginName, requestURI, Joiner.on(SymbolConstant.COMMA).join(currentAuthorityUrl));
		// 超级管理员 全部都可以访问
		if (StringUtils.equals(currentLoginName, SysConstant.SUPER_MANAGER_LOGIN_NAME)) {
			return true;
		}
